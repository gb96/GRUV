...
def convert_flac_to_wav(filename, sample_frequency):
    ...
    if not os.path.exists(new_path):
        os.makedirs(new_path)
    new_name = new_path + '/' + orig_filename + '.wav'
    cmd = 'sox {0} {1} channels 1 rate {2}'.format(quote(filename),
                                                   quote(new_name),
                                                   sample_frequency)
    os.system(cmd)
    return new_name
...
def time_blocks_to_fft_blocks(blocks_time_domain):
    ...
    for block in blocks_time_domain:
        ...
        new_block = np.concatenate((np.real(fft_block),
                                    np.imag(fft_block)))
...
def convert_wav_files_to_nptensor(directory, block_size, max_seq_len, out_file, max_files=20, useTimeDomain=False):
    ...
    std_x = np.sqrt(np.mean(np.mean(np.abs(x_data - mean_x)**2, axis=0),
                            axis=0))
...
def convert_nptensor_to_wav_files(tensor, indices, filename, useTimeDomain=False):
    ...
    for i in indices:
        ...
        save_generated_example(filename + str(i) + '.wav',
                               chunks, useTimeDomain=useTimeDomain)
    return None
...
def save_generated_example(filename, generated_sequence,
                           useTimeDomain=False, sample_frequency=44100):
    if useTimeDomain:
